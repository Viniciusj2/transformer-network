...
class PMTDataset(Dataset):
    ...
    def _create_linear_encoding(
        ...
        """
        Create a linear encoding for input values using evenly spaced embeddings.
        Args:
            values (np.ndarray): Input values to encode.
            min_val (float): Minimum value for encoding.
            max_val (float): Maximum value for encoding.
        Returns:
            torch.Tensor: Linear encoding of input values.
        """
        ...
        values = values[:, np.newaxis]  # Shape: (len(values), 1)
        
        # Apply linear scaling
        encoding = values * linear_scale  # Broadcasting across the embedding dimension
    ...
    def _process_event(self, event_num: int) -> torch.Tensor:
        with tables.open_file(self.h5_filename, mode='r') as file:
            ...
            for i in range(n_pmts):
                ...
                # Populate tensor
                # First value is the raw ground truth charge
                event_tensor[i, 0] = raw_q
                
                # Embedding starts from the next index
                event_tensor[i, 1:self.embedding_dim] = q_emb.squeeze()[1:]
                
                # Delta time starts at embedding_dim with raw ground truth
...
class ModifiedTransformerModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # More explicit tensor slicing
        q_vals = x[:, :, 0:self.embedding_dim]  # First embedding_dim for charges
        dt_vals = x[:, :, self.embedding_dim:2*self.embedding_dim]  # Next embedding_dim for delta t
        x_pos = x[:, :, 2*self.embedding_dim:3*self.embedding_dim]  # X position
        y_pos = x[:, :, 3*self.embedding_dim:4*self.embedding_dim]  # Y position
        z_pos = x[:, :, 4*self.embedding_dim:]  # Z position
        
        # Extract first element of each embedding
        q_extract = q_vals[:, :, 0:1]  # Shape: [batch_size, num_pmts, 1]
        dt_extract = dt_vals[:, :, 0:1]  # Shape: [batch_size, num_pmts, 1]
