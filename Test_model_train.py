...
# Keep the PMT positions as they are...
# PMT positions
pmt_positions = [
...
def preprocess_pmt_data_for_transformer_with_full_embeddings(h5_filename, pmt_positions, embedding_dim=8):
    """
    Preprocesses PMT data with sparse encoding for all features (Q, Δt, X, Y, Z)
    
    Args:
        h5_filename: String path to the h5 file
        pmt_positions: List of dictionaries containing PMT positions
    ...
    # Initialize arrays
    pmt_coords = np.array([[p['x'], p['y'], p['z']] for p in pmt_positions])
    
    # Normalize coordinates and features
    coords_mean = pmt_coords.mean(axis=0)
    coords_std = pmt_coords.std(axis=0)
    pmt_coords_normalized = (pmt_coords - coords_mean) / coords_std
    
    # Create embeddings for coordinates
    x_embeddings = create_sparse_encoding(pmt_coords_normalized[:, 0], embedding_dim)
    y_embeddings = create_sparse_encoding(pmt_coords_normalized[:, 1], embedding_dim)
    z_embeddings = create_sparse_encoding(pmt_coords_normalized[:, 2], embedding_dim)
...
def train_model(model, train_data, num_epochs=20, learning_rate=1e-3, batch_size=64, log_dir="runs_2"):
    # Setup TensorBoard
...
def evaluate_model(model, test_data, writer=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
...
def main():
    ...
    embedding_dim = 8  # New embedding dimension for each feature
    num_heads = 8      # Increased for larger embedding dimension
    num_layers = 3     # Slightly deeper network
    
    # Process the data with new embedding scheme
    processed_data = preprocess_pmt_data_for_transformer_with_full_embeddings(
        h5_filename, pmt_positions, embedding_dim=embedding_dim)
    
    # Split data
    train_data, test_data = train_test_split(
        processed_data.numpy(), test_size=0.2, random_state=42)
